...
class Stack:
	...
	def push(self):
		if Stack.top == Stack.n:
			...
			Stack.top +=1
			Stack.dict1[Stack.top] = self.tkSimpleDialog.askstring('Dictonary', 'Enter yhe element')
			Label(self.f, bg="red",width=14,height=2,text= Stack.dict1[Stack.top]).grid(row = ((Stack.n+2)-Stack.top), column=2)
			
	def pop(self):
		if Stack.top == 0:
			tkMessageBox.showinfo("CAN NOT POP", "stack is empty")
		else:
			Label(self.f, bg="green",width=14,height=2).grid(row = ((Stack.n+2)-Stack.top), column=2)
			Stack.top -=1
...
class Stackepliment(Stack):
	...
	def tope(self):
		if Stack.top == 0:
		  tkMessageBox.showinfo("TOP", "0")
		else:
			tkMessageBox.showinfo("TOP",Stack.top)
	def generate(self):
	# Define UI
		...
		Button(self.f,text= "TOP",width=8,height=2, bg = "blue",command=self.tope).grid(row=0, column=3)
